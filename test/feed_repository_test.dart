...
class FakeFilterBuilder extends Fake
  ...
  PostgrestTransformBuilder<List<Map<String, dynamic>>> select([
    ...
    return FakeTransformBuilderList(
  ...
  Future<S> then<S>(
    ...
    return onValue(
      returnValue == null
          ? []
          : (returnValue is List
                ? returnValue as List<Map<String, dynamic>>
                : [returnValue as Map<String, dynamic>]),
    );
  }
}

class FakeTransformBuilderList extends Fake
  ...
  PostgrestTransformBuilder<Map<String, dynamic>?> maybeSingle() {
    return FakeTransformBuilderMap(
      errorToThrow: errorToThrow,
      returnValue: returnValue,
    );
  }
...
class FakeTransformBuilderMap extends Fake
...
void main() {
  ...
  setUp(() {
    ...
    registerFallbackValue(
      LocalBookmark(bookmarkId: '', postId: '', userId: '', isSynced: false),
    );
  });
  ...
  group('FeedRepository Bookmark Tests (White Box)', () {
    // TC-WB-01
    test(
      'TC-WB-01: toggleBookmark - Adds new bookmark when it does not exist',
      () async {
    ...
    test('TC-WB-02: toggleBookmark - Deletes existing bookmark', () async {
      repository.mockUserId = 'user123';
      final postId = 'post1';
      final existingBookmark = LocalBookmark(
        bookmarkId: 'b-123',
        postId: postId,
        userId: 'user123',
        isSynced: true,
    ...
    // TC-WB-03
    test(
      'TC-WB-03: toggleBookmark - Skips gracefully when user not logged in',
      () async {
        repository.mockUserId = null; // No user
        final postId = 'post1';

        // Call it normally. We expect it to finish without crashing.
        await repository.toggleBookmark(postId);

        // Verify that because they weren't logged in, it never touched the database
    ...
    test('TC-WB-04: getOfflineBookmarks - Retrieves correctly', () {
      ...
      final bookmark1 = LocalBookmark(
        bookmarkId: 'b1',
        postId: 'p1',
        userId: 'user123',
        isSynced: true,
      );
      final bookmark2 = LocalBookmark(
        bookmarkId: 'b2',
        postId: 'p2',
        userId: 'user123',
        isSynced: true,
      ...
      final post1 = CachedPost(
        postId: 'p1',
        cachedData: '',
        cachedAt: DateTime.now(),
      );
      final post2 = CachedPost(
        postId: 'p2',
        cachedData: '',
        cachedAt: DateTime.now(),
      );
